...
export class CCMResUtils {
        ...
        if (attachNode) {
            ...
            if (!ret) {
                if (autoCreate) {
                    ...
                    return CCMResUtils.getResKeeper(attachNode.parent!, autoCreate);
                }
            }
            return ret;
        }
    ...
    */
    public static assignWith(srcAsset: Asset, targetNode: Node, autoCreate?: boolean): any {
        let keeper = CCMResUtils.getResKeeper(targetNode, autoCreate);
        if (keeper && srcAsset instanceof Asset) {
            keeper.cacheAsset(srcAsset);
            return srcAsset;
        } else {
            CCMLogger.getInstance().error(`assignWith ${srcAsset} to ${targetNode} failed`);
            return null;
        }
    }
    ...
    public static instantiate(prefab: Prefab): Node {
        ...
        let keeper = CCMResUtils.getResKeeper(node, true);
        if (keeper) {
    ...
    public static getCallStack(popCount: number): string {
        ...
        let pos = CCMResUtils.findCharPos(ret!, '\n', popCount);
        if (pos > 0) {
            ret = ret!.slice(pos);
        }
        return ret!;
